...
def load_mapping(filename):
    ...
    try:
        ...
        print(f"Error: Could not parse mapping file '{filename}'. Check the file format.")
        return None
...
def categorize_transactions(transactions_df, mapping):
    ...
    for index, row in transactions_df.iterrows():
        ...
        if not matched:
            ...
            keyword = input("Enter keyword to associate with this description: ").strip().lower()
...
def modify_csv(input_file, output_file, mapping_file):
    ...
    try:
        ...
        print(f"Error: Could not parse input file '{input_file}'. Check the file format.")
        return
    ...
    try:
        ...
        print("Error: Could not parse dates. Please ensure the date format is YYYY-MM-DD.")
        return
    ...
    if updated_map:
        try:
            mapping_df = pd.DataFrame.from_dict(mapping, orient='index', columns=['Category', 'Subcategory'])
    ...
    df = df[['', 'Month', 'Date', 'Description', '$', 'Amount', 'Category', 'Sub-Category']]
